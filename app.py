...
@st.cache_resource
def load_all():
    # Load pickled mappings
    # drug_to_index: {drugbank_id -> int index}
    ...
    df = df[df['clean_name'].notna()].copy()
    df = df[df['clean_name'].str.strip() != ''].copy()
    df = df[df['drugbank_id'].isin(drug_to_index.keys())].copy()
    df.reset_index(drop=True, inplace=True)

    # Build name → drugbank_id mapping
    # Use first occurrence per clean_name
    name_to_id = {}
    ...
    # Build edge_index from CSV interactions
    src_list, dst_list = [], []
    for _, row in df.iterrows():
        id1 = row['drugbank_id']
    ...
    if len(src_list) == 0:
        # fallback: identity edges to avoid crash
        n = len(drug_to_index)
